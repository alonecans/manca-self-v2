...
✿➤ Ketik : ${prefix}daftar`
	...
	nuy.on('chat-update', async (mek) => {
		...
		if (messagesLink.includes('://chat.whatsapp.com/')) {
            ...
            if (isGroupAdmins) return reply(`Kamu *${pushname2}* adalah admin group kamu tidak akan di kiᴄk`)
            nuy.updatePresence(from, Presence.composing)
        ...
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: ftoko, mimetype: 'audio/mp4', ptt:true})
        }
        ...
        if (messagesLink.includes('#miku')){
		nuy.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/wibu.mp3')
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        }
        if (messagesLink.includes('#help')){
		nuy.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/help.mp3')
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        }
        if (messagesLink.includes('#loli')){
		nuy.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/wibu.mp3')
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        }
        if (messagesLink.includes('#animegirl')){
		nuy.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/wibu.mp3')
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        }
        if (messagesLink.includes('#animeboy')){
		nuy.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/wibu.mp3')
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        }
        if (messagesLink.includes('#animecouple')){
		nuy.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/wibu.mp3')
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        }
        if (messagesLink.includes('#naruto')){
		nuy.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/naruto.mp3')
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        }
...
❏ ẉww.instagram.com/thenay_xploit_
╰──────────
╭─「 *BOT INFO* 」
❏ Sc Ori By: FxSx
❏ Recode By: NuyFaa
...
nuy.sendMessage(from, menu, image, {quoted: ftoko, caption: captionnya})
            break
            case 'help':
...
case 'igpost': 				
           ...
           if (isBanned) return reply(mess.only.benned)   
				...
				reply(mess.wait)
				anu = await fetchJson(`http://zekais-api.herokuapp.com/kusonime?query=${body.slice(10)}`)
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 nuy.sendMessage(from, hehre, image, {quoted: ftoko, caption: 'Nih Kak'})
} else {
  reply('Reply Imagenya Kak')
}
break
...
quoted: mek, caption: `cogan ganteng neh:v`
  ...
  break
			...
			if (!isUser) return reply(mess.only.userB)
				...
				case 'daftar':
					nuy.updatePresence(from, Presence.composing)
					if (isUser) return reply('*Kamu Sudah Terdaftar Di Manca Bot!*')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
					...
					nuy.sendMessage(from, daftarimg, image, {quoted: ftoko, caption: captionnya})
					break
                    case 'profile':
    				nuy.updatePresence(from, Presence.composing)
					if (!isUser) return reply(mess.only.userB)
					...
					nuy.sendMessage(from, buff, image, {quoted: ftoko, caption: profile})
					break
				case 'bahasa':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				case 'info':
				    ...
				    nuy.sendMessage(from, daca, image, {quoted: ftoko, caption: teks})
					break 
				case 'totaluser':
					nuy.updatePresence(from, Presence.composing) 
					if (isBanned) return reply(mess.only.benned)    
					...
					nuy.sendMessage(from, teks.trim(), extendedText, {quoted: ftoko, contextInfo: {"mentionedJid": user}})
					break
				case 'blocklist':
					teks = 'List Block :\n'
...
break
           case 'galaxtext2':
               ...
               glaktx = body.slice(11)
               reply(mess.wait)
               galax = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/galaxywallpaper?apikey=7f5a6556983b0bf183028c20&text=${glaktx}`)
               nuy.sendMessage(from, galax, image, {quoted: mek})
               await limitAdd(sender)
               break
                case 'glittergold':
                ...
                case 'silverplaybutton':
                    ...
                    if (!isUser) return reply(mess.only.userB)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/${command}?apikey=7f5a6556983b0bf183028c20&text=${txt}`)
                    nuy.sendMessage(from, buffer, image, {quoted: mek})
                    break
               case 'epep':
                   if (isBanned) return reply(mess.only.benned)    
	...
	break
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
            ...
            break  
		        ...
		        nuy.sendMessage(from, buffer, image, {quoted: ftoko, caption: `*NAME* : ${groupName}\n*MEMBER* : ${groupMembers.length}\n*ADMIN* : ${groupAdmins.length}\n*DESK* : ${groupDesc}`})
                break
				case 'trendtwit':
					nuy.updatePresence(from, Presence.composing) 
                     if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
				    ...
				    nuy.sendMessage(from, yeh, text, {quoted: ftoko})
			        break
				case 'hidetag':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							reply(mess.wait)
							nuy.sendMessage(from, buff, audio, {quoted: ftoko, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					await limitAdd(sender) 
...
// only grup fitur anime
           ...
           case 'animeboy':
			    ...
			    if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					...
					goo = await getBuffer(tu)
					nuy.sendMessage(from, goo, image, { quoted: mek })
					break
              case 'animegirl':
					if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
// akhir fitur anime
              ...
              case 'randomkpop':
			   ...
			   if (isLimit(sender)) return reply(limitend(pushname2))
			   reply(mess.wait)
              	    if (args.length < 1) return reply('*MAAF MENU INI DI OFF IN SAMA OWNER!!*')
                    teks = body.slice(9)
           ...
           case 'fototiktok':
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ribet = body.slice(12)
                    anu = await fetchJson(`https://docs-jojo.herokuapp.com/api/tiktokpp?user=${ribet}` , {method: 'get'})
			        buffff = await getBuffer(anu.result)
                    nuy.sendMessage(from, buffff, image, {quoted: mek})
			        await limitAdd(sender)
					break
				case 'tiktokstalk':
					try {
...
// premium user
       ...
       case 'play':
          case 'playmp3':
				...
				goo = body.slice(7)
                data = await fetchJson(`https://lindow-api.herokuapp.com/api/ytplay?judul=${goo}&apikey=LindowApi`, {method: 'get'})
                reply(mess.wait)
